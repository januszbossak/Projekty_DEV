...
**Projekty:** `Organizacja-DEV/Dokumentacja-organizacyjna`, `Klienci/WIM/Repozytorium-plikow-DMS`, `Klienci/LOT/JRWA`, `cross-cutting/Wzmiankowanie-w-komentarzach`, `Moduly/Ustawienia-systemowe`, `cross-cutting/Interfejs-sprawy`, `Moduly/Edytor-procesow/Edytor-formularzy`, `Klienci/Lewiatan/Comarch-HUB`, `Moduly/e-Nadawca`, `cross-cutting/Interfejs-sprawy/Podglad-szablonow`

> 🛡️ Zweryfikowano przez Note Reviewer: 2025-12-06
...
| Repozytorium - frontend | `Klienci/WIM/Repozytorium-plikow-DMS` | 🔄 W trakcie | Filip już trochę frontendu zrobił |
...
**Kontekst i cel:**
Damian Kaminski potrzebuje wsparcia architektonicznego do zaprojektowania backendu dla repozytorium plików. Janusz Bossak wskazał, że Damian, jako Product Owner, powinien skupić się na wymaganiach biznesowych (przypadki użycia), a nie na szczegółach technicznych implementacji, którymi powinien zająć się architekt systemowy.
Po 8 dniach sprintu powinna być rozkminiona architektura i struktura backendu - to będzie duży wkład. Nie dajemy jeszcze do robienia w rozumieniu pisania kodu, tylko niech się zastanowią porządnie.

**Zakres prac:**

| Zadanie | Osoba | Estymacja | Zależności |
|---------|-------|-----------|------------|
| Ustalenie architektury aplikacji (odrębna aplikacja vs część AMODIT, komunikacja z systemem AMODIT, użytkownicy, przechowywanie plików, mikroserwisy) | **Piotr** | - | Spotkanie z Damianem (30 minut), nagrywanie, przejście przez transkrypcję, uzupełnienie dokumentacji. Piotr ma opowiadać, a Damian ma go podpytywać. |
...
**Projekt:** `Integracje/Integracja-KSeF`

**Kontekst i cel:**
Adrian jest przeładowany (KSeF, macOS, Comarch Hub). Nierealne jest w 8 dni nawet minimalne MVP z tego złożyć. Trzeba okroić do tego, czego faktycznie potrzebujemy biznesowo, żeby nie "doktoryzował" się z konektora KSeF. Nagrać "zeznania".
...
**Kontekst i cel:**
Janusz rozpracowuje temat, on ma ustalić "czy coś robimy". Okazuje się, że nie jest tak, że każdy widzi całe JRWA. Nakłada się matryca kompetencji wynikająca ze struktury organizacji. Zgodnie z ochroną danych osobowych, ludzie powinni mieć dostęp do tej części JRWA, która ich dotyczy. Księgowy nie może sobie założyć teczki w radzie nadzorczej.
...
| Zadanie | Osoba | Estymacja | Zależności |
|---------|-------|-----------|------------|
| Implementacja 4-eyes w ustawieniach systemowych | **Przemek** (Ania miała być, ale Kamil mówił o Przemku) | - | - |
...
**Kontekst i cel:**
Cały czas jest kwestia czytelności. Chciałbym to zaopiekować w przyszłym sprincie. Miejmy na uwadze, że ten sprint zamyka paczkę grudniową, więc powinniśmy stabilizować wersję, a nie dorzucać nowe rzeczy.
...
**Kontekst i cel:**
Nowy temat. Adrian go rozkminia na poziomie koncepcyjnym. Może przekaże to Łukaszowi. Na razie jednak Łukasz jest na urlopie. Musimy poczekać do poniedziałku. Pytanie: czy ten Comarch Hub musi robić Adrian, czy może Łukasz Brocki?
...
**Kontekst i cel:**
Poprawki w e-Nadawcy, które ma Adrian. Może przejąć Łukasz Brocki? Ktoś musi się też nauczyć e-Doręczeń.

**Zakres prac:**

| Zadanie | Osoba | Estymacja | Zależności |
|---------|-------|-----------|------------|
| Poprawki w e-Nadawcy | **Łukasz Brocki** (potencjalnie od Adriana) | - | Po powrocie z urlopu |
...
|---------|----------|--------|--------------|
| Piotr ma zająć się architekturą repozytorium (fundamenty, konstrukcja), Mateusz opracowuje backend na tych założeniach | `Klienci/WIM/Repozytorium-plikow-DMS` | ✅ Zatwierdzone | Damian nie ma wystarczającej wiedzy technicznej, Piotr jako architekt systemowy ma wymyślać architekturę i pilnować jej spójności. Janusz: "Uczmy Piotra jednej rzeczy: żeby włączał mikrofon i gadał." |
| Dokumentacja projektowa: projekt minimalnie składa się z trzech plików: 1. Uzasadnienie biznesowe, 2. Rozbicie na MVP, 3. Plik architektoniczno-techniczny | `Organizacja-DEV/Automatyzacja-dokumentacji-AI` | ✅ Zatwierdzone | Potrzeba miejsca do przechowywania i edycji dokumentacji projektowej dostępnej dla wszystkich. |
| Uprawnienia w repozytorium oparte o przypisanie do działów (podobnie jak w JRWA) | `Klienci/WIM/Repozytorium-plikow-DMS` | ✅ Zatwierdzone | Zgodnie z ochroną danych osobowych, ludzie powinni mieć dostęp do tej części, która ich dotyczy. |
...
|---------------|---------|-------|-----------|------------|
| Adrian przeładowany (KSeF, macOS, Comarch Hub) | `Integracje/Integracja-KSeF`, `Klienci/WIM/Podpis-kwalifikowany-SignApp-macOS`, `Klienci/Lewiatan/Comarch-HUB` | Wysoki | Okrojenie KSeF do potrzeb biznesowych, przekazanie Comarch Hub Łukaszowi po urlopie | **Damian**, **Adrian** |
| Mateusz przeładowany (komunikator + repozytorium) | `Klienci/WIM/Komunikator`, `Klienci/WIM/Repozytorium-plikow-DMS` | Wysoki | Mateusz ma się mocno zaangażować, nawet po godzinach (rozlicza się godzinowo - "jak mu powiemy"). | **Mateusz** |
| Nie ma co zakładać, że repozytorium wyprodukujemy w jeden sprint | `Klienci/WIM/Repozytorium-plikow-DMS` | Średni | Po 8 dniach powinna być rozkminiona architektura i struktura backendu - to będzie duży wkład. | - |
...
- **Spotkania:** 
  ...
  - Spotkanie Damian + Adrian - rozpracowanie KSeF (okrojenie do potrzeb biznesowych), nagrać "zeznania".
