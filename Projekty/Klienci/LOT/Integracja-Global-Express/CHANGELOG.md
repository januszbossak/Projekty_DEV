# CHANGELOG

Historia ustaleń i zmian dla projektu.

---

## 2025-12-01 | Sprint review
**Źródło:** [Notatki/Gotowe-notatki-archiwum/2025-12-01 Sprint review.md]
**Kategoria:** #Funkcjonalność #Dokumentacja

- Zaimplementowano funkcje reguł: otwarcie, wezwanie kuriera, śledzenie, anulowanie.
- Ograniczenia: MVP bez opcjonalnych parametrów.
- Testy u klienta (PGL LOT).

---

---

## 2025-11-28 | Planowanie sprintu
**Źródło:** [Notatki/Gotowe-notatki-archiwum/2025-11-28 Planowanie sprintu.md]
**Kategoria:** #Funkcjonalność #Dokumentacja

- Funkcje integracji ukończone.
- Trwają prace nad dokumentacją i testami.
- SM Integracja (firma kurierska) - do zrobienia przez analogię.

---

## 2025-11-21 | Planowanie Sprintu
**Źródło:** [Notatki/Gotowe-notatki-archiwum/2025-11-21 Planowanie sprintu.md]
**Kategoria:** #Funkcjonalność #Zadanie #Postęp

- Łukasz Brocki pracuje nad integracją z kurierami UPS i Global Express.
- Dane potrzebne do integracji zostały pozyskane.
- Global Express ma szansę zostać ukończony w bieżącym sprincie.

---

## 2025-11-17 | Planowanie sprintu
**Źródło:** [Notatki/Gotowe-notatki-archiwum/2025-11-17 Planowanie sprintu.md](../../../../Notatki/Gotowe-notatki-archiwum/2025-11-17%20Planowanie%20sprintu.md)
**Kategoria:** #Funkcjonalność #Zadanie #MVP

- Ustalono realizację w drugiej kolejności (po Comarch Hub).
- Zakres MVP: realizacja funkcji dostępnych w API (wysyłka, odbiór, status, anulowanie).
- Oczekiwanie na pytania od Łukasza Brockiego (przekazane).

---

## 2025-11-07 | Planowanie Sprintu
**Źródło:** [Notatki/Gotowe-notatki-archiwum/2025-11-07 Planowanie sprintu.md]
**Kategoria:** #Funkcjonalność #Zadanie #Problem

- Integracja z Global Express jest jednym z trzech zadań dla LOT do końca roku.
- Należy rozpoznać, czy Global Express posiada API.
- Brak przypisanej osoby do zadania.
- Brak informacji o API to obecna blokada.

---